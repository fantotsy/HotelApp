        ...
        if (session != null) {
            ...
            if (guestURIs.contains(uriPath)) {
                if (role != null && role.equals("guest")) {
                    ...
                    String viewPage = UrlGetter.getInstance().getUrl(UrlGetter.ERROR_PAGE);
                    viewPage = viewPage.replace("/HotelApp", "");
                ...
                if (role == null || request.getParameter("logout") != null) {
                    ...
                    if (role.equals("admin")) {
                        String viewPage = ActionsGetter.getInstance().getAction(ActionsGetter.ADMIN);
                        viewPage = viewPage.replace("/HotelApp", "");
                        request.getRequestDispatcher(viewPage).forward(request, response);
                    } else {
                        String viewPage = ActionsGetter.getInstance().getAction(ActionsGetter.GUEST);
                        viewPage = viewPage.replace("/HotelApp", "");
                        request.getRequestDispatcher(viewPage).forward(request, response);
                    }
                }
